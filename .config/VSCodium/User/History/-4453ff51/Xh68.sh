...
MENU_OPTIONS=(
    "Refresh" "Enable Wi-Fi" "Disable Wi-Fi" \
    "Network Info" "Scan Networks" "Connect" "Disconnect"
)
...
# Close other notifiation after a new function
notify() {dunstctl close-all ; notify-send $1 $2;}

# Function to disconnect from the currently connected network
function disconnect() {
    iwctl station "$INTERFACE" disconnect
    dunstctl close-all ; notify-send "Network Disconnected!!" "You are now disconnected to ${ssid[$1]}..."
}
...
function connect_to_network() {
    local selected_ssid="${ssid[$1]}" ; notify-send "Connecting..." "Attempting to access $selected_ssid!!"
    ...
    if [[ -n "$known" ]]; then
        ...
        if [[ -z "$connection_output" ]]; then
            dunstctl close-all ; notify-send "Connection Was Successful!!" "You are now connected to $selected_ssid!!" ; return
        else
            dunstctl close-all ; notify-send "Connection Was Unsuccessful!!" "$selected_ssid may not be available for a while, please try again!!" ; return
        fi
    else
        ...
        if [[ -n "$connection_output" ]]; then
            dunstctl close-all ; notify-send "Connection Was Unsuccessful!!" "The password entered may be wrong or empty, please try again!!"
        else
            dunstctl close-all ; notify-send "Connection Was Successful!!" "You are now connected to $selected_ssid!!"
        fi
    fi
}
...
function scan() {
    ...
    while (( selected_wifi_index == 1 )); do
        dunstctl close-all ; notify-send "Scanning..." "For nearby networks!!"
